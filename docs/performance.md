...
I tested on 10 September 2026 using a 16 inch MacBook Pro with an Apple M4 Max, 16 CPU cores, 40 GPU cores and 48 GB unified memory. macOS 26.3.1 (a), CrossOver 26.2, MoltenVK 1.4.1 privateapi, Steam app 392160 build 23660954. I used my existing 64 bit Steam bottle with MSync enabled and graphics backend Auto. I did not record the exact DLC/mod list or an instrumented frame time capture.

In my tests, gameplay was stable, with no visible artifacts or major freezes in two scenarios. My very developed save with megastations, thousands of player and faction ships, and large fleets felt like roughly 45 to 60 FPS. My early save with far fewer entities felt like 100 FPS or more. The Steam overlay was black, so **neither range is measured FPS**. My screenshots show the map and an external flight view. I have not verified the same results in every cockpit view, fleet battle or station interior. The ship counts are my estimates, and my comparison with a powerful desktop PC comes from playing the game rather than controlled measurements.

My settings menu and local configuration both show **1740×1129**. My Mac panel is **3456×2234**. Retina scaling and screenshot pixel dimensions do not establish the game's actual framebuffer size. I have not verified 45 to 60 FPS at full panel resolution. CrossOver explains its high resolution mode in the [user guide](https://support.codeweavers.com/en_US/crossover-mac-user-guide).

My configuration also contains `gamespeed=0.5`. I did not record whether Reduced Speed Mode was active during these sessions; the stored value alone does not prove it was enabled. I need to record the actual simulation speed in future tests. This is another reason I describe these results as impressions rather than a controlled benchmark. Egosoft introduced [Reduced Speed Mode as an accessibility option](https://wiki.egosoft.com/X4%20Foundations%20Wiki/Game%20Updates%20and%20Patch%20History/X4%20Foundations%207.xx%20Changelogs/).

My Activity Monitor snapshots show X4 at about 11.91 GB memory, total system memory use about 39.99 GB, green memory pressure and no swap on my 48 GB machine. These are snapshots, not peak requirements. Total CPU idle percentage does not rule out a busy simulation thread; I took the separate CPU snapshots at different moments.

## Reproduce my graphics profile
...
I suggest these profiles as starting points to test, not measured minimums or purchasing guarantees. I have only tested on M4 Max / 48 GB. The [original forum report](https://www.codeweavers.com/support/forums/general/?t=27;msg=346291#c6) also reports launch on M4 Pro without an FPS benchmark.
...
| Match my setup | M4 Max 16 CPU / 40 GPU, 48 GB | 1740×1129, profile above | Stable play in my tests; FPS subjective |
...
Allow room for the installed game and DLC, the original CrossOver, about 1 GB for its copied app and temporary download/copy space. Prefer at least **5 GB free beyond the already installed game** for this installation, plus normal macOS memory/swap headroom. My game installation occupies about 32 GB, but another DLC set may differ. Game requirements are listed by [Egosoft on Steam](https://store.steampowered.com/app/392160/X4_Foundations/); Windows GPU minimums are not equivalent Mac requirements.
